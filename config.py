...
#    UI는 모든 층위(도메인·분야·세분류)를 한 검색창에서 섞어 복수 선택.
#    층위가 섞이면 세분류 단위로 정규화해 한 필터 키로 OR(앱 build_field_filter).
#    옵션 목록은 app.get_taxonomy()가 OpenAlex /domains·/fields·/subfields 로 채움.
# ============================================================
DOMAIN_KR = {"1": "생명과학", "2": "사회과학", "3": "자연과학", "4": "보건과학"}

# 기본으로 미리 선택해 둘 분야 — 환경변수/Secrets 로 개인별 설정(공개 기본값은 제한 없음).
#   OPENALEX_DEFAULT_IDS: '|' 로 구분한 도메인/분야/세분류 ID (예: "3321" 또는 "33|17")
#   층위 무관 — id 값으로 매칭해 자동 선택됨.
...
INST_CAVEAT = (
    ...
    "논문 수가 과대/과소집계될 수 있으니, 기관 간 단순 비교 시 이 점을 감안해 해석하세요."
)
