...
## 입력 3가지 → 같은 분석
- **검색**: 전 분야(OpenAlex 도메인4·필드26·세분류252를 한 검색창에서 **층위 섞어 복수 선택**) · 키워드(동의어 OR `|`) · 검색범위·구문일치 · 저자(이름·ORCID·ID) · 기관(autocomplete·ROR) · 연도
...
2. (선택) 앱 **Settings → Secrets**:
   ...
   # 기본으로 미리 선택해 둘 분야(선택):
   OPENALEX_DEFAULT_IDS = "3321"   # 도메인/분야/세분류 ID, '|' 로 복수 (예: "3321|17")
   ```

## 분야 분류
OpenAlex Topics 4계층(Domain 4 · Field 26 · Subfield 252 · Topic). UI의 한 검색창에서 **모든 층위를 섞어 복수 선택**할 수 있습니다 — 예: `[세분류] Public Administration` + `[분야] Computer Science`. 층위가 섞이면 자동으로 **세분류 단위로 정규화해 한 필터 키로 OR**(OpenAlex는 서로 다른 필터 키 사이 OR을 막아서 우회). 옵션 목록은 앱이 OpenAlex에서 동적으로 가져옵니다. 기본 분야는 `OPENALEX_DEFAULT_IDS` 로 설정(미설정 시 제한 없음).
